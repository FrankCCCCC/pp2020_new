...
int main(int argc, char** argv) {
    ...
    t.show_rec("io");
    t.show_rec("total");
    // printf("During %lf\n", ((double) (e_t - s_t)) * 1000 / CLOCKS_PER_SEC);
    const char *order[4] = {"cpu", "lock", "io", "total"};
...
void *worker(void *worker_arg_v){
    ...
    Timer thread_t;
    if(!is_enable_show){thread_t.disable_show();}
    char timer_name[20] = {0};
    sprintf(timer_name, "thread_%d_load", thread_id);
    // printf("Thread %d Created(Self: %d)\n", thread_id, pthread_self());
    Task *task = NULL;

    thread_t.start_rec(timer_name);
    for(;(!pool->is_finish) || (!is_task_queue_empty(pool));){
        int is_has_task = 0;
    ...
    thread_t.pause_rec(timer_name);
    thread_t.show_rec(timer_name);
    const char *order[1] = {timer_name};
    thread_t.report("./measure/report.csv", order, 1);
    // printf("Thread: %s\n", timer_name);

    // printf("Thread %d During %lf\n", thread_id, ((double) (w_e_t - w_s_t)) * 1000 / CLOCKS_PER_SEC);
    pthread_exit(NULL);
}
...
void start_pool(ThreadPool* pool){
    ...
    for(int i = 0; i < get_threads_num(pool); i++){
        ...
        // char timer_name[20] = {0};
        // sprintf(timer_name, "thread_%d_load", i);
