...
#include <omp.h>
#include <pthread.h>
#include "../../libs/timer_mpi/timer_mpi.h"
...
int main(int argc, char** argv) {
    ...
    t.enable_mpi();
    if(argv[10][0] == 'N'){t.disable_show(); is_enable_show=0;}
    else if(argv[10][0] == 'Y'){ is_enable_show=1;}
    t.start_rec("total");
	...
	if(rank == MASTER_RANK){
        ...
        t.pause_rec("total");
        t.pause_rec("io");
        t.set_rec("comm", comm_thread_t_time + t.get_rec("comm"));
        ...
        t.show_rec("io");
        t.show_rec("total");
        const char *order[4] = {"cpu", "comm", "io", "total"};
        out_file = argv[9];
...
void *master_thread_task_comm(void *arg){
    ...
    comm_thread_t_time = comm_thread_t.get_rec("comm");
...
void slave(){
    ...
    Timer slave_t;
    slave_t.enable_mpi();
    char load_timer_name[20] = {0};
    sprintf(load_timer_name, "process_%d_load", rank);
    
    slave_t.start_rec(load_timer_name);
	for(;;){
    ...
    slave_t.pause_rec(load_timer_name);
    printf("%lf, ", slave_t.get_rec(load_timer_name));
