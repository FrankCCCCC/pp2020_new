...
__device__ void assignAij(int *dist, int (*AM)[Share_Mem_Size * Share_Mem_Size], int vertex_num, int Round, int block_internal_start_x, int block_internal_end_x, int block_internal_start_y, int block_internal_end_y){
...
__device__ void assignCkj(int *dist, int (*CM)[Share_Mem_Size * Share_Mem_Size], int vertex_num, int Round, int block_internal_start_x, int block_internal_end_x, int block_internal_start_y, int block_internal_end_y){
...
__device__ void assignBik(int *dist, int (*BM)[Share_Mem_Size * Share_Mem_Size], int vertex_num, int Round, int block_internal_start_x, int block_internal_end_x, int block_internal_start_y, int block_internal_end_y){
...
__device__ void assignBik_r(int *dist, int (*BM)[Share_Mem_Size * Share_Mem_Size], int vertex_num, int Round, int block_internal_start_x, int block_internal_end_x, int block_internal_start_y, int block_internal_end_y){
    for (int k = Round * B + threadIdx.y; k < (Round + 1) * B && k < vertex_num; k+=blockDim.y) {
        for (int i = block_internal_start_x + threadIdx.x; i < block_internal_end_x; i+=blockDim.x) {
    ...
    for (int k = Round * B; k < (Round + 1) * B && k < vertex_num; k++) {
        for (int i = block_internal_start_x + threadIdx.x; i < block_internal_end_x; i+=blockDim.x) {
            ...
            for (int j = block_internal_start_y + threadIdx.y; j < block_internal_end_y; j+=blockDim.y) {
                ...
                // }
                // a[(i - block_internal_start_x) * Share_Mem_Row_Size + (j - block_internal_start_y)] = min(d, a[(i - block_internal_start_x) * Share_Mem_Row_Size + (j - block_internal_start_y)]);
                (*AM)[(i - block_internal_start_x) * Share_Mem_Row_Size + (j - block_internal_start_y)] = min_u(d, (*AM)[(i - block_internal_start_x) * Share_Mem_Row_Size + (j - block_internal_start_y)]);
            }
        }
        __syncthreads();
    }
...
__device__ void relax_r(int (*AM)[Share_Mem_Size * Share_Mem_Size], int (*BM)[Share_Mem_Size * Share_Mem_Size], int (*CM)[Share_Mem_Size * Share_Mem_Size], int vertex_num, int Round, int block_internal_start_x, int block_internal_end_x, int block_internal_start_y, int block_internal_end_y){
    // Relax Path
    for (int k = Round * B; k < (Round + 1) * B && k < vertex_num; k++) {
        for (int i = block_internal_start_x + threadIdx.x; i < block_internal_end_x; i+=blockDim.x) {
...
__device__ void relax_r_async(int (*AM)[Share_Mem_Size * Share_Mem_Size], int (*BM)[Share_Mem_Size * Share_Mem_Size], int (*CM)[Share_Mem_Size * Share_Mem_Size], int vertex_num, int Round, int block_internal_start_x, int block_internal_end_x, int block_internal_start_y, int block_internal_end_y){
    // Relax Path
    for (int k = Round * B; k < (Round + 1) * B && k < vertex_num; k++) {
        for (int i = block_internal_start_x + threadIdx.x; i < block_internal_end_x; i+=blockDim.x) {
            int bv = (*BM)[(k - Round * B) * Share_Mem_Row_Size + (i - block_internal_start_x)];
