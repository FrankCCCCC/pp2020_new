    ...
    for (int k = Round * B; k < (Round + 1) * B && k < vertex_num; k++) {
        for (int i = block_internal_start_x + threadIdx.x; i < block_internal_end_x; i+=blockDim.x) {
            ...
            for (int j = block_internal_start_y + threadIdx.y; j < block_internal_end_y; j+=blockDim.y) {
                ...
                a[(i - block_internal_start_x) * Share_Mem_Row_Size + (j - block_internal_start_y)] = min_u(d, a[(i - block_internal_start_x) * Share_Mem_Row_Size + (j - block_internal_start_y)]);
            }
        }
        __syncthreads();
    }
...
__device__ void relax_r_async(int a[Share_Mem_Size * Share_Mem_Size], int b[Share_Mem_Size * Share_Mem_Size], int c[Share_Mem_Size * Share_Mem_Size], int vertex_num, int Round, int block_internal_start_x, int block_internal_end_x, int block_internal_start_y, int block_internal_end_y){
    // Relax Path
    for (int k = Round * B; k < (Round + 1) * B && k < vertex_num; k++) {
        for (int i = block_internal_start_x + threadIdx.x; i < block_internal_end_x; i+=blockDim.x) {
            int bv = b[(k - Round * B) * Share_Mem_Row_Size + (i - block_internal_start_x)];
