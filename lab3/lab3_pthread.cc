...
unsigned long long calc_pixs_k(int rank){
    unsigned long long pixels = 0;
    unsigned long long rank_u = (unsigned long long)rank;
    int is_stop = 0;

    for (unsigned long long x = seg_len_e + rank_u; x <= seg_len_s && !is_stop; x+=(unsigned long long)ncpus) {
        unsigned long long y = 0;
        // x is target segment length
        // Calculate start point of thickness
        unsigned long long thick_s = ceil(sqrtl(r_squr - (x * x)));
        ...
        if(thick_s > r_half){
            break;
        }else if(thick_s == r_half){

        }else if(thick_e >= r_half){
            thick_e = r_half;
            is_stop = 1;
        }
        y = (((thick_e - thick_s) % k) * (x % k)) % k;
